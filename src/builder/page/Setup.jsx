...
const CATEGORY_LABELS = {
  contact:  __( "Contact",  "flowforms" ),
  feedback: __( "Feedback", "flowforms" ),
  "lead-gen": __( "Lead Gen", "flowforms" ),
  survey:   __( "Survey",   "flowforms" ),
  general:  __( "General",  "flowforms" ),
};

export default function Setup({ className }) {
  const templates = flowformsBuilderData.templates ?? [];
  ...
  const createBlank = async () => {
    ...
    try {
      const res = await fetch(flowformsBuilderData.apiUrl + "/forms", {
        method: "POST",
        headers: {
      ...
      window.location.href = `${flowformsBuilderData.builderUrl}&form_id=${data.post_id}`;
    } catch (err) {
  ...
  const useTemplate = async (slug) => {
    ...
    setLoadingSlug(slug);
    try {
      const res = await fetch(flowformsBuilderData.apiUrl + "/forms/from-template", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-WP-Nonce": flowformsBuilderData.nonce,
  ...
  const openPreview = async (slug) => {
    ...
    try {
      const res = await fetch(
        `${flowformsBuilderData.apiUrl}/templates/${slug}/preview-url`,
        {
          headers: { "X-WP-Nonce": flowformsBuilderData.nonce },
        },
      );
      const data = await res.json();
      if (!res.ok || !data.preview_url)
        throw new Error(data.message || __( "Could not load preview.", "flowforms" ));
        ...
        <div className="bg-white rounded-xl max-w-5xl mx-auto px-4 py-6 sm:p-8">
          ...
          <div className="mb-6 pb-6 border-b border-gray-200">
            <h1 className="text-xl font-semibold text-gray-900 tracking-tight">
              { __( "Select a template", "flowforms" ) }
            </h1>
            <p className="mt-2 text-sm text-gray-500">
              { __( "Start from a template or build from scratch.", "flowforms" ) }
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-6">
            <div className="relative flex-1 max-w-xs">
              ...
              <input
                ...
                placeholder={ __( "Search templates…", "flowforms" ) }
          ...
          {filtered.length === 0 && search ? (
            <div className="text-center py-16 text-sm text-gray-500">
              { __( "No templates match", "flowforms" ) } <strong>"{search}"</strong>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <BlankCard
                name={ __( "Blank form", "flowforms" ) }
                description={ __( "Start from scratch with an empty form", "flowforms" ) }
        ...
        >
          {loading ? (
            <>
              ...
              { __( "Creating…", "flowforms" ) }
            </>
          ) : (
            __( "Start from scratch", "flowforms" )
          )}
        </button>
      </div>
    </div>
    ...
    <div className="group relative bg-white rounded-xl border border-gray-200 hover:border-gray-300 hover:shadow-sm transition-all flex flex-col overflow-hidden">
      <div className="w-full aspect-3/2 bg-gray-50 flex items-center justify-center border-b border-gray-100">
        {thumbnailUrl ? (
          <img
            src={thumbnailUrl}
            alt={ sprintf( /* translators: %s: template name */ __( "%s template screenshot", "flowforms" ), name ) }
...
function TemplatePreviewModal({ url, onClose }) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={ __( "Template preview", "flowforms" ) }
    >
      {/* Toolbar */}
      <div className="flex-none flex items-center justify-between h-14 px-4 border-b border-gray-200 bg-white">
        ...
        <button
          ...
          aria-label={ __( "Close preview", "flowforms" ) }
        >
          ...
          {!loaded && (
            <div className="absolute inset-0 flex items-center justify-center bg-gray-50 rounded-2xl">
              <div className="flex flex-col items-center gap-3 text-gray-400">
                ...
                <span className="text-sm">{ __( "Loading preview…", "flowforms" ) }</span>
              </div>
            </div>
          )}
          <iframe
            ref={iframeRef}
            src={url}
            title={ __( "Template preview", "flowforms" ) }
