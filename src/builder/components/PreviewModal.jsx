...
const DEVICES = [
  { id: "desktop", label: __( 'Desktop', 'flowforms' ), width: "100%",   height: "100%",   icon: <ComputerDesktopIcon width={20} height={20} /> },
  { id: "tablet",  label: __( 'Tablet', 'flowforms' ),  width: "768px",  height: "1024px", icon: <DeviceTabletIcon width={20} height={20} /> },
  { id: "mobile",  label: __( 'Mobile', 'flowforms' ),  width: "390px",  height: "844px",  icon: <DevicePhoneMobileIcon width={20} height={20} /> },
];
...
export default function PreviewModal({ open, onClose }) {
  ...
  // already baked into flowformsBuilderData.previewUrl).
  useEffect(() => {
    if (open && formId) {
      setSrcUrl(flowformsBuilderData.previewUrl || "");
      setLoaded(false);
    }
  }, [open, formId]);
  ...
  return (
    ...
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={ __( 'Form preview', 'flowforms' ) }
    >
      {/* ── Toolbar ─────────────────────────────────────────────────────── */}
          ...
          <div className="flex items-center gap-2">
            ...
            {srcUrl && (
              <a
                ...
                title={ __( 'Open in new tab', 'flowforms' ) }
              >
      ...
      <div className="flex-1 flex items-center justify-center overflow-hidden p-4 pt-1">
        ...
        >
          ...
          {srcUrl ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center bg-gray-100 rounded-2xl">
              <p className="text-sm text-gray-500">{ __( 'Preview URL not available. Save the form first.', 'flowforms' ) }</p>
            </div>
          )}
        </div>
      </div>
