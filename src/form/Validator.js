...
export function validate( question, answer, previewMode ) {
	...
	if ( settings.required && isEmpty( answer, type ) ) {
		return window.flowformPublicData?.i18n?.required ?? __( 'This field is required.', 'wpflowforms' );
	}
	...
	switch ( type ) {
		case 'email':
			if ( settings.confirmEmail ) {
				...
				if ( ! isValidEmail( emailVal ) ) {
					return window.flowformPublicData?.i18n?.email ?? __( 'Please enter a valid email address.', 'wpflowforms' );
				}
				if ( emailVal !== confirmVal ) {
					return window.flowformPublicData?.i18n?.emailMismatch ?? __( 'Email addresses do not match.', 'wpflowforms' );
				}
			} else {
		...
		case 'number': {
			...
			if ( isNaN( num ) ) return window.flowformPublicData?.i18n?.number ?? __( 'Please enter a valid number.', 'wpflowforms' );
			...
			if ( hasMin && num < Number( settings.min ) ) {
				return ( window.flowformPublicData?.i18n?.numberMin ?? __( 'Value must be at least {min}.', 'wpflowforms' ) ).replace( '{min}', settings.min );
			}
			if ( hasMax && num > Number( settings.max ) ) {
				return ( window.flowformPublicData?.i18n?.numberMax ?? __( 'Value must be at most {max}.', 'wpflowforms' ) ).replace( '{max}', settings.max );
			}
			break;
		}
		...
		case 'rating': {
			...
			if ( ! Number.isInteger( val ) || val < 1 || val > steps ) {
				return window.flowformPublicData?.i18n?.rating ?? __( 'Please select a valid rating.', 'wpflowforms' );
			}
			break;
		}
		...
		case 'multiple_choice':
			if ( ! Array.isArray( answer ) || answer.length === 0 ) {
				return window.flowformPublicData?.i18n?.selection ?? __( 'Please make a selection.', 'wpflowforms' );
			}
			break;
		...
		case 'checkboxes': {
			...
			if ( minSel > 0 && answer.length < minSel ) {
				return ( window.flowformPublicData?.i18n?.checkboxesMin ?? __( 'Please select at least {count} options.', 'wpflowforms' ) ).replace( '{count}', minSel );
			}
			if ( maxSel > 0 && answer.length > maxSel ) {
				return ( window.flowformPublicData?.i18n?.checkboxesMax ?? __( 'Please select at most {count} options.', 'wpflowforms' ) ).replace( '{count}', maxSel );
			}
			break;
		}
		...
		case 'yes_no':
			if ( answer !== 'yes' && answer !== 'no' ) {
				return window.flowformPublicData?.i18n?.yesNo ?? __( 'Please select yes or no.', 'wpflowforms' );
			}
			break;
	}
