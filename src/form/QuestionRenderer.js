...
export function renderQuestion( question, answer, error, design, onChange ) {
	...
	title.textContent = content.title || __( 'Untitled question', 'wp-flowforms' );
...
function buildInput( type, content, settings, answer, alignment, onChange, questionId ) {
	switch ( type ) {
		...
		default: {
			...
			fb.textContent = __( 'This question type is not supported.', 'wp-flowforms' );
			return fb;
		}
	}
}
...
function buildRating( settings, answer, onChange ) {
	...
	for ( let i = 1; i <= steps; i++ ) {
		...
		btn.type = 'button';
		btn.setAttribute( 'aria-label', sprintf( __( 'Rate %1$s out of %2$s', 'wp-flowforms' ), i, steps ) );
